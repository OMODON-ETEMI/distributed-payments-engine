...
SELECT id, idempotency_key, scope, request_hash, response_code, response_body, locked_at, expires_at, created_at, updated_at FROM idempotency_keys WHERE scope = @scope AND idempotency_key = @idempotency_key LIMIT 1 FOR NO KEY UPDATE;
...
SELECT id, customer_id, external_ref, account_number, account_type, status, currency_code, ledger_normal_side, metadata, opened_at, closed_at, created_at, updated_at, deleted_at FROM accounts WHERE id = @id::uuid LIMIT 1 FOR NO KEY UPDATE;
...
WHERE account_id = ANY(@account_id::uuid[]) AND currency_code = @currency_code::char(3) AND balance_kind = @balance_kind
ORDER BY id 
FOR NO KEY UPDATE;
